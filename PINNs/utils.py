...
class system_dynamics():
    ...
    def generate_data(self, v_file_name, w_file_name, pt_file_name): #Temporary becasue we dont have W yet!!Plz add w_file_name later
        ...
        all_y_cor = []
        all_t = []
        total_sim_time = data_V.shape[0] #The shape of data_V is (time, no. of nodes)
        time_step = 0.1

        for i in range(0, total_sim_time):
            time = i*time_step
            all_t.append(time)

        #all_x_cor and all_y_cor will have repeating x and y coordinate
        for i in range(0, coordinates.shape[0]):
            z_cor = coordinates[i][2]
            if z_cor == 100:
                x_cor = coordinates[i][0]/1000
                y_cor = coordinates[i][1]/1000
                all_x_cor.append(x_cor)
                all_y_cor.append(y_cor)

        x = sorted(list(set(all_x_cor)))
        y = sorted(list(set(all_y_cor)))
        ...
        for i in range(0, len(y)):
            V_array_list.append(data_V[:, 0+i*len(x):len(x)+i*101])
            W_array_list.append(data_W[:, 0+i*len(x):len(x)+i*101])
            
        Vsav = np.stack((V_array_list), axis=0)
        Vsav = np.transpose(Vsav, (2, 0, 1))
        Wsav = np.stack((W_array_list), axis=0)
        Wsav = np.transpose(Wsav, (2, 0, 1))
        ...
        V = Vsav.reshape(-1, 1)
        W = Wsav.reshape(-1, 1)    
        return np.hstack((X, Y, T)), V, W, Vsav, all_t
