...
def plot_results(Vsav, observe_train, v_train, observe_test, v_pred, all_t, fig_name, animation):
    ...
    sorted_array = stacked_array[np.lexsort((stacked_array[:, 1], stacked_array[:, 0], stacked_array[:, 2]))] #Rearranging the array's order based on x,y and t
    ...
    sorted_array_obv = stacked_array_obv[np.lexsort((stacked_array_obv[:, 1], stacked_array_obv[:, 0], stacked_array_obv[:, 2]))]
