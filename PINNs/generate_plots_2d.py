...
def plot_2D_cell(data_list, dynamics, model, fig_name):
    ...
    cell_x = (dynamics.max_x*0.75// 1000)*1000
    cell_y = (dynamics.max_y*0.75// 1000)*1000
...
def generate_2D_animation(dynamics, model, fig_name):
    ...
    def get_animation_data(i):
        ...
        v_pred = model.predict(data)[:,0:1]
        Z = np.zeros((grid_size,grid_size))
        for i in range(grid_size_):
            Z[i,:] = (v_pred[(i*grid_size_):((i+1)*grid_size_)]).reshape(-1)
        return Z
